...
**`web` does not depend on `core`.** The GA engine never enters the browser bundle — the browser only ever receives *data* about a run, never the machinery that produces it. Adding `@genebaer/core` to `apps/web/package.json` would silently pull the entire engine into the client bundle. Don't.

It imports `@genebaer/shared-types`, and `@genebaer/vision/augment` — **that subpath specifically**. `vision` does depend on `core`, so importing the package *root* from the browser would drag the engine in behind it, along with the PNG encoder's `node:zlib`. The `./augment` entry point exists to prevent exactly that: `dist/augment.js` has no imports at all, and the engine is verifiably absent from `.next/` output. If you add another browser-facing module to `vision`, give it its own subpath rather than widening this one.
...
5. **A problem may fix its genome length, and a mismatch is rejected at construction.** `FitnessProblem.requiredGenomeLength` is non-null for Weasel (one gene per target character), MDS (one per vertex) and ImagePrompt (set by its representation); encodings report `genomeSize` from their parsed params. The engine throws a `RangeError` naming the expected length and the param to change, surfacing as a 422. It reads params rather than generating a genome, so the check never advances the seeded RNG. Problems returning `null` — OneMax, Sphere, Rastrigin — work at any length and are skipped. This used to be accepted silently: Weasel scored against `min(genome.length, target.length)`, so a wrong-sized run optimised a prefix of the target and looked healthy (genebaer-1os).
...
7. **A queued evaluator needs a running server.** It resolves its queue from module scope, because the registry constructs operators with params only and cannot inject services. Using one outside a server rejects with a clear message rather than hanging.
8. **A WS lease frame is large.** A population of 40 at 64x64 RGB encodes to ~1.97 MB of JSON, not the ~491 KB the raw pixel count suggests - JSON writes each byte as decimal text. `maxPayload` is 8 MB for this reason; at 128x128 a batch of 40 uses ~7.87 MB, so bigger runs must claim smaller batches.
9. **Real model inference is not covered by the test suite.** CLIP scoring uses a swappable backend and the tests inject a fake, so the contract, caching and failure paths are covered but "the model produces a useful gradient" is not. Measured separately - see `docs/experiments/`.
